      ...
      <div className="w-full max-w-lg bg-white/80 backdrop-blur-md border border-white/30 rounded-3xl shadow-2xl p-6 sm:p-10 flex flex-col">
        ...
        <div className="max-h-[75vh] overflow-y-auto pr-2 space-y-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            <InputField label="NIC Number" value={nic} onChange={setNic} placeholder="Enter your NIC number" />
            <InputField label="Full Name" value={fullName} onChange={setFullName} placeholder="Enter your full name" />
            <InputField label="User Name" value={userName} onChange={setUserName} placeholder="Enter your user name" />
            <SelectField label="Gender" value={gender} onChange={setGender} options={["Male", "Female", "Other"]} />
            <InputField label="District" value={district} onChange={setDistrict} placeholder="Enter your district" />
            <InputField label="Age" value={age} onChange={setAge} placeholder="Enter your age" type="number" />
...
function SelectField({ label, value, onChange, options }) {
  return (
    <div>
      <label className="block text-sm font-semibold text-[#B2B0E8] mb-2 drop-shadow">{label}</label>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        required
